...
struct Result{
	...
	double labelXpos;
	double labelYpos;
	Result() :angle(-1), leftDistance(-1), rightDistance(-1), labelValid(false), labelXpos(0),labelYpos(0){};
	void reset(){
		angle = -1; leftDistance = -1; rightDistance = -1; labelValid = false; labelXpos = 0; labelYpos = 0;
	}
};
...
private:
	...
	std::vector<Line_Detection> lineGetInf; //计算得到相关信息后的线段
	...
	cv::Mat warp3DInv;
	cv::Mat warp3DBias;
	cv::Mat_<cv::Vec3b> warpedImage;
	double lengthAll;
	inline void drawLine();
	void findPt(const double &A, const double &B, const double &C, std::vector<cv::Point2d> &pt);
	bool sampleLineToFit(int index1, int index2, double &A, double &B, double &C);
	bool loadModel();
	bool loadGauss();
	...
	void getPointInImageFrom3D(double x, double y,double &u, double &v){   //可用于得到兴趣区域的范围
		...
		cv::Mat axisWorld = cv::Mat(3, 1, CV_64FC1, axisInWorld);
	...
	double rolLabel, colLabel;
	cluter labelGMM; //初始化时需要载入label的模式文件
	bool boxDetect();
...
public:
	...
	cv::Mat outPutImg;
	void processImage(const cv::Mat &image, bool labelDetectFlag, Result &detectResult,double interestAngle=-1,bool trainMode = false, int imageIndex = -1);
};