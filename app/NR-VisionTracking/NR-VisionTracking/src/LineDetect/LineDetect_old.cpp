...
inline cv::Vec3d getInterplatedVec3b(const cv::Mat &M, const double &xd, const double &yd){
	assert(M.channels() == 3);
...
void LineDetector::fuseCluter(){
	...
	while (chaneFlag){
		...
		if (lineCluters.size() > 200)
			lineCluters.erase(lineCluters.begin() + 200, lineCluters.end());
		...
		while (index1 < lineCluters.size()) //待融合直线
		{
			...
			while (index2 < lineCluters.size())
			{
				...
				sampleLineToFit(index1, index2, lineCluters[index1].meanA, lineCluters[index1].meanB
...
void LineDetector::sampleLineToFit(int index1, int index2, double &A, double &B, double &C){
