...
#define ROIHEIGHT 330 //感兴趣的区域高度
#define MININF 1E-10
#define FUSE_DIST_THRES 0.5
#define FUSE_ANGLE_THRES 0.0005
#define LINE_LENGTH_THRES 200
...
void LineDetector::processImage(const Mat &inputImageOrg, bool labelDetectFlag, Result &detectResult, bool trainMode, int imageIndex){
	detectResult.reset();
	clearAll();
	bool ShowFlag = false;	
...
inline void LineDetector::drawLine(){
	...
	if (leftNearestIndex>=0)
		line(outPutImg, usefulCluters[leftNearestIndex].bottomPixelLoc, usefulCluters[leftNearestIndex].upPixelLoc, Scalar(0, 255, 0), 1);
	if (rightNearestIndex>=0)
		line(outPutImg, usefulCluters[rightNearestIndex].bottomPixelLoc, usefulCluters[rightNearestIndex].upPixelLoc, Scalar(0, 255, 0), 1);
}
...
bool LineDetector::boxDetect(){ //检测贴在直线上的标签，首先该标签必须位于直线上
	...
	//double startX=
	double stepX = cos(angle / 180 * PI); //确保每一步像素
	double stepY = sqrt(1 - stepX*stepX);
	double stepXL = -stepY; double stepYL = stepX;

	double lenRect = 2*boxLen, widthRect = 2*boxWidth; //需要检测的区域的大小
	int boderWidth = widthRect / 2;
	if (rolLabel < 0){
		rolLabel = (2 * boderWidth - 1) / 2;
		colLabel = lenRect / 2;
	}
	//vector<Mat_<double>> scoreArray(pointCluter.size());
	//Engine *ep;
	//ep = engOpen("\0");
	//imshow("warpedImageMask", outputMask);
	//waitKey(30);
	//cv::imshow("warpedImage", warpedImage);
	//cv::waitKey(0);
	labelYPos = 100000000000;
	for (int i = 0; i < pointCluter.size();){ //计算该中心到最近直线的距离，并将最近直线的斜率赋予当前pointCluter,根据sumX和Y求得所在直线的方程；
		Mat_<double> scoreArray = 1000 * Mat::ones(2 * boderWidth - 1, lenRect, CV_64FC1);
		Mat_<double> Xpos = 1000 * Mat::ones(2 * boderWidth - 1, lenRect , CV_64FC1);
		Mat_<double> Ypos = 1000 * Mat::ones(2 * boderWidth - 1, lenRect, CV_64FC1);

		double startX = pointCluter[i].meanX - stepX*lenRect/2;
		double startY = pointCluter[i].meanY - stepY*lenRect / 2;
		double endX = pointCluter[i].meanX + stepX*lenRect / 2;
		double endY = pointCluter[i].meanY + stepY*lenRect / 2;

		/*Mat tempImage;
		warpedImage.copyTo(tempImage);

		circle(tempImage, Point2f(startX, startY), 3, Scalar(0, 0, 255), 1);
		circle(tempImage, Point2f(endX, endY), 3, Scalar(0, 255, 0), 1);
		cv::imshow("warpedImage", tempImage);
		cv::waitKey(0);*/

		double curX = startX, curY = startY;

		int nowColScoreArray = -1;
		while ((stepX < 0) == (curX > endX) && (stepY < 0) == (curY > endY)){
			curX += stepX; curY += stepY;
			++nowColScoreArray;
			if (nowColScoreArray == scoreArray.cols)
				break;
			if (curX<boderWidth || curX>warpedImage.cols - boderWidth
				|| curY<boderWidth || curY>warpedImage.rows - boderWidth){
				continue;
			}
			int nowRowScoreArray = 0;
			for (int k = -boderWidth+1; k < boderWidth; ++k){
				double x = curX + k* stepXL;
				double y = curY + k* stepYL;
				Vec3d p = getInterplatedVec3b(warpedHSV, x, y);
				Mat err = Mat(p).reshape(1).t() - labelGMM.center;
				Mat res = err*labelGMM.covinv*err.t();
				scoreArray(nowRowScoreArray, nowColScoreArray) = res.at<double>(0, 0);
				Xpos(nowRowScoreArray, nowColScoreArray) = x;
				Ypos(nowRowScoreArray, nowColScoreArray) = y;
				++nowRowScoreArray;
			}
		}
		//cout << scoreArray[i] << endl;
		//cout << scoreArray << endl;
		GaussianBlur(scoreArray, scoreArray, Size(5, 5), 3);
		//Mat ss2;
		//scoreArray.copyTo(ss2);
		integral(scoreArray, scoreArray); //计算好积分图，求解使得积分数值最小的积分点的位置
		
		Mat_<double> temp = 1000000 * Mat::ones(2 * boderWidth - 1, lenRect + 2, CV_64FC1); //
		double minCol, minRow;
		double minScore = 1000000;
		{
			for (int i = 0; i < scoreArray.cols - boxLen; ++i)
			for (int j = 0; j < scoreArray.rows - boxWidth; ++j)
			{
				temp(j, i) = scoreArray(j + boxWidth, i + boxLen) - scoreArray(j, i + boxLen) //得到最佳的距离
					- scoreArray(j + boxWidth, i) + scoreArray(j, i);
				if (temp(j, i) < minScore){
					minScore = temp(j, i);
					minCol = i;
					minRow = j;
				}
		...
		temp /= boxLen*boxWidth;
		pointCluter[i].score = temp(minRow, minCol);
		if (pointCluter[i].score > 12){ //无效的检测块
			pointCluter[i] = pointCluter.back();
			pointCluter.pop_back();
			continue;
		}

		minRow += (boxWidth - 1) / 2.0 + rolLabel; 
		minCol += (boxLen-1) / 2.0+colLabel;
		minCol /= 2;  minRow /= 2;
		colLabel = minCol; rolLabel = minRow; //移动平滑

		pointCluter[i].refinedX = 0.5 * (Xpos(minRow, minCol) + pointCluter[i].meanX);  //+colLabel //+ pointCluter[i].meanX
		pointCluter[i].refinedY = 0.5 * (Ypos(minRow, minCol) + pointCluter[i].meanY); //+ rolLabel // + pointCluter[i].meanY

		pointCluter[i].refinedX *= ratio;
		pointCluter[i].refinedY *= ratio;
		...
		//minMaxLoc(ss2, &minVal, &maxVal);
		//imwrite("aaaa.png", ss2);
		//ofstream of("1.txt");
		//of << ss2;
		//ss2 -= minVal;
		//ss2 *= 254 / (maxVal - minVal);
		//Mat tempScoreArray;
		//ss2.convertTo(tempScoreArray, CV_8UC1);
		//circle(tempScoreArray, Point2d(minCol, minRow), 2, Scalar(255, 0, 0), 1);
		//imshow("score", tempScoreArray);
		//waitKey(10);

		//Mat tempImage;
		//warpedImage.copyTo(tempImage);

		//circle(tempImage, Point2f(pointCluter[i].refinedX, pointCluter[i].refinedY), 3, Scalar(0, 0, 255), 1);
		//circle(tempImage, Point2f(pointCluter[i].meanX, pointCluter[i].meanY), 3, Scalar(0, 255, 0), 1);
		//cout << "得分：" << pointCluter[i].score << endl;
		//cv::imshow("warpedImage", tempImage);
		//cv::waitKey(0);
		++i;
	}
	...
	/*for (int i = 0; i < pointCluter.size(); ++i){
		circle(warpedImage, Point2f(pointCluter[i].refinedX, pointCluter[i].refinedY), 3, Scalar(255, 0, 0), 1);
		circle(warpedImage, Point2f(pointCluter[i].meanX, pointCluter[i].meanY), 3, Scalar(0, 0, 255), 1);
	}*/
	...
	//imwrite((num2str(1) + "angle.jpg").c_str(), warpedImage);

/*	vector<vector<Point2d> > pointCluter;
	pointCluter.resize(usefulCluters.size());

	double toBoderThres = SAMPLE_POINT_TO_BORDER_3D / ratio;
	for (int i = 0; i < usefulCluters.size(); ++i){
		double sx, sy, ex, ey; //起始点和终止点在warpImage中的坐标
		int boderWidth = 3;
		getPoint3D(usefulCluters[i].bottomPixelLoc.x, usefulCluters[i].bottomPixelLoc.y + imageHeight - ROIHEIGHT, sx, sy);
		sx += biasX; sx /= ratio;
		sy += biasY; sy /= ratio;

		getPoint3D(usefulCluters[i].upPixelLoc.x, usefulCluters[i].upPixelLoc.y + imageHeight - ROIHEIGHT, ex, ey);  
		ex += biasX; ex /= ratio;
		ey += biasY; ey /= ratio;

		//cv::line(warpedImage, Point2d(sx, sy), Point2d(ex, ey), Scalar(255, 0, 0));
		double len = sqrt((ex - sx)*(ex - sx) + (ey - sy)*(ey - sy));
		double stepX = (ex - sx) / len;
		double stepY = (ey - sy) / len;
		double stepXL = -stepY; double stepYL = stepX;
		double curX = sx;
		double curY = sy;
		while (((stepX < 0) == (curX >= ex)) && (stepY < 0) == (curY >= ey)){
			if (curX<toBoderThres || curX>warpedImage.cols - toBoderThres
				|| curY<toBoderThres || curY>warpedImage.rows - toBoderThres){
				curX += stepX; curY += stepY;
				continue;
			}

			for (int k = 1; k < boderWidth; ++k){
				double x1 = curX - (baseDetectArea + k)* stepXL;
				double y1 = curY - (baseDetectArea + k)*stepYL;
				double x2 = curX + (baseDetectArea + k)* stepXL;
				double y2 = curY + (baseDetectArea + k)* stepYL;

				//cout << int(x1 + 0.5) << " " << int(y1 + 0.5) << endl;
				//cout << int(x2 + 0.5) << " " << int(y2 + 0.5) << endl;

				Vec3b curColor1 = getInterplatedVec3b(warpedImage, x1, y1);
				Vec3b curColor2 = getInterplatedVec3b(warpedImage, x2, x2);
				if ((curColor1[0]<LABEL_COLOR_THRES &&curColor1[1]<LABEL_COLOR_THRES&&curColor1[2]<LABEL_COLOR_THRES) ||
					(curColor1[0]>255 - LABEL_COLOR_THRES &&curColor1[1]>255 - LABEL_COLOR_THRES&&curColor1[2]>255 - LABEL_COLOR_THRES)){
					pointCluter[i].push_back(Point2d(x1, y1));
					//warpedImage(y1 + 0.5, x1 + 0.5) = Vec3b(0, 0, 255);
				}
					
				if ((curColor2[0]<LABEL_COLOR_THRES &&curColor2[1]<LABEL_COLOR_THRES&&curColor2[2]<LABEL_COLOR_THRES) ||
					(curColor2[0]>255 - LABEL_COLOR_THRES &&curColor2[1]>255 - LABEL_COLOR_THRES&&curColor2[2]>255 - LABEL_COLOR_THRES)){
					pointCluter[i].push_back(Point2d(x2, y2));
					//warpedImage(y2 + 0.5, x2 + 0.5) = Vec3b(0, 0, 255);
				}
			}
			curX += stepX; curY += stepY;
		}
	}
	//cout << pointCluter[i].size() << endl;
	imwrite((num2str(1) + "angle.jpg").c_str(), warpedImage);*/
}